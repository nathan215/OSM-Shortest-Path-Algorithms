...
<p align="center"><img src="Analysis/figures/runtime_own_vs_networkx.png" width="900" alt="Mean runtime of own implementations vs NetworkX per algorithm, city and scenario"></p>

Mean runtime per algorithm over the 100 random pairs in each `Analysis/algorithm_results_<city>_<scenario>.csv` (log scale; generated by `Analysis/plot_runtime.py`). Two things stand out: on long routes the hand-written versions are within about 1.5x of NetworkX, but on short routes over a full metro graph the own Dijkstra is 38-61x slower, because it keeps expanding the whole graph instead of stopping at the target, and A* narrows the gap since its heuristic prunes that search.
